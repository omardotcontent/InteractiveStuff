...
public final class ScriptItemHandler {
    ...
    public static ItemStack apply(ItemStack stack, String componentId, String rawValue) {
        if (stack == null || componentId == null || rawValue == null) return stack;

        // Auto-fix namespace
        if (!componentId.contains(":")) componentId = "minecraft:" + componentId;
    ...
    private static <T> void applyDecoded(ItemStack stack, ComponentType<T> type, NbtElement nbt) {
    ...
    /* ===================== STRING -> NBT ===================== */
    // [Keep your existing coerce/parsing methods here, they look fine for script inputs]
    private static NbtElement coerce(String input) {
        input = input.trim();
        if (input.equals("null")) return null;
        if (input.equalsIgnoreCase("true") || input.equalsIgnoreCase("false")) return NbtByte.of(Boolean.parseBoolean(input));
        if (input.matches("-?\\d+")) return NbtInt.of(Integer.parseInt(input));
        if (input.matches("-?\\d+\\.\\d+")) return NbtDouble.of(Double.parseDouble(input));
        if (input.startsWith("\"") && input.endsWith("\"")) return NbtString.of(input.substring(1, input.length() - 1));
        if (input.startsWith("[") && input.endsWith("]")) return parseList(input);
        if (input.startsWith("{") && input.endsWith("}")) return parseCompound(input);
